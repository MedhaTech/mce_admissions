...
With good wishes";
	...
	public function admissionsletterlateral($encryptId)
	{
		...
		if ($this->session->userdata('logged_in')) {
			...
			$pdf->SetFont('Arial', '', 10);
			$pdf->MultiCell(0, 5, '       ' . $nameData . ' has sought admission to the 3rd Semester B.E course in ' . $this->admin_model->get_dept_by_id($data['admissionDetails']->dept_id)["department_name"] . ' branch at Malnad College of Engineering, Hassan for the year 2024-25.', 0, 1);

			$pdf->Ln(4);

			$pdf->MultiCell(0, 5, "       There is likelihood of some seats remaining vacant after the counseing of Government CET lateral process for 3rd semester and if so your request for admission will be considered. If for any reasons seats are filled by the Government CET lateral process, you have no right to seek admissions.Your admission is strictly subjected to the approval of the Government.");
			$pdf->Ln(4);
	...
	public function consolidated_report($download = 0)
{
    if ($this->session->userdata('logged_in')) {
		...
		$print_fields = array(
			...
			'Guardian Annual Income', 'Guardian EmailID', 'Guardian Mobile 1', 'SSLC', 'SSLC Institution Board','SSLC Institution Name', 
			'SSLC Institution Address', 'SSLC Institution City', 'SSLC Year of Passing', 'SSLC Institution State', 
			'SSLC Institution Country','SSLC Medium of Instruction', 'SSLC Percentage',
			'SSLC Register Number','', 'Education Level 2', 'Education Level 2 Institution Board', 
			'Education Level 2 Institution Name','Education Level 2 Institution Address', 'Education Level 2 Institution City', 'Education Level 2 Year of Passing',
			 'Education Level 2 Institution State','Education Level 2 Institution Country', 'Education Level 2 Medium of Instruction',  'Education Level 2 Percentage', 
		    'Education Level 2 Register Number');
		$this->table->set_heading($print_fields);
