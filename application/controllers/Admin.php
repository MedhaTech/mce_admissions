...
With good wishes";
	...
	function updatequota($encryptId)
	{
		if ($this->session->userdata('logged_in')) {
			$session_data = $this->session->userdata('logged_in');
			$data['id'] = $session_data['id'];
			...
			$id = base64_decode($encryptId);
			$admissionDetails = $this->admin_model->getDetails('admissions', $id)->row();
			$data['stream'] = $admissionDetails->stream_id;
			$data['admissionDetails'] = $this->admin_model->getDetails('admissions', $id)->row();
			...
			if ($data['stream'] == 1) {
				...
				$data['quota_options'] = array(" " => "Select") + $this->globals->quota();
				$data['subquota_options'] = array(" " => "Select") + $this->globals->sub_quota();
			} elseif ($data['stream'] == 2) {
				
				$data['course_options'] = array(" " => "Select") + $this->pgcourses();
				$data['quota_options'] = array(" " => "Select") + $this->globals->quota();
				$data['subquota_options'] = array(" " => "Select") + $this->globals->sub_quota();
			} else {
				$data['course_options'] = array(" " => "Select Branch") + $this->phdcourses();
				$data['quota_options'] = array(" " => "Select") + $this->globals->quota();
				$data['subquota_options'] = array(" " => "Select") + $this->globals->sub_quota();
			}
			...
			$this->form_validation->set_error_delimiters('<div class="error">', '</div>');
			// $data['admissions'] = $this->admin_model->get_details_by_id($id, 'id', 'admissions');
			$this->form_validation->set_rules('course', 'Department Id', 'required');
			$this->form_validation->set_rules('quota', 'Quota', 'required');
			...
			if ($this->form_validation->run() === FALSE) {
				...
				$data['action'] = 'admin/updatequota/' . $encryptId;
				...
				$this->admin_template->show('admin/updatequota', $data);
			} else {
				$course = $this->input->post('course');
				$corpus = $this->input->post('corpus_fee')?? 0;
				$category_claimed = $this->input->post('category_claimed');
				$category_allotted = $this->input->post('category_allotted');
				$total_tution_fee = $this->input->post('total_tution_fee');
				$total_university_fee = $this->input->post('total_university_fee')?? 0;
				...
				$result = $this->admin_model->updateDetailsbyfield('student_id',$id, $updateDetails1, 'fee_master');
				$this->admin_model->update_multiple_payment_structure($id);
				if ($result) {
					$this->session->set_flashdata('message', 'Quota Details updated successfully...!');
					$this->session->set_flashdata('status', 'alert-success');
				} else {
					$this->session->set_flashdata('message', 'Oops something went wrong please try again.!');
					$this->session->set_flashdata('status', 'alert-warning');
